...
type AjvInstance = {
  ...
  getSchema: (schemaRef: string) => ValidateFunction | undefined;
};

const Ajv2020Ctor = Ajv2020Import as unknown as new (options: Record<string, unknown>) => AjvInstance;
...
const DELEGATE_AUTH_SCHEMA_ID =
  "https://example.com/schemas/delegate-authentication/bundle.schema.json";
const DELEGATE_PAYMENT_SCHEMA_ID =
  "https://example.com/schemas/delegate-payment/bundle.schema.json";
...
function rewriteCartRefs(value: JsonValue): JsonValue {
  ...
  if (value && typeof value === "object") {
    ...
    for (const [key, nested] of Object.entries(value)) {
      clone[key] = rewriteCartRefs(nested as JsonValue);
    }
    return clone;
  }
...
function webhookEventSchema(): JsonObject {
  return {
...
function compileOrThrow(ajv: AjvInstance, schemaRef: string): ValidateFunction {
...
export class ProtocolValidator {
  private readonly validators: Record<ValidatorName, ValidateFunction>;

  constructor(validators: Record<ValidatorName, ValidateFunction>) {
    this.validators = validators;
  }

  validate(name: ValidatorName, data: unknown): ValidationResult {
