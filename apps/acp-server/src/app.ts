...
import {
  getRequiredHeaders,
  loadHeaderRequirements,
  type HeaderRequirements,
  type OpenApiFile
} from "./protocol/openapiHeaders.js";
import { createProtocolValidator, type ValidationResult, type ValidatorName } from "./protocol/validation.js";
import { createId, createStore, nowIso, snapshot, type InMemoryStore } from "./store.js";
...
type ErrorType = "invalid_request" | "processing_error" | "service_unavailable" | "rate_limit_exceeded";
...
function firstValidationParam(validation: ValidationResult): string | undefined {
...
function logOperation(operationId: string, req: Request, extra: Record<string, unknown> = {}): void {
...
function applyEchoHeaders(req: Request, res: Response): void {
...
function enforceHeaders(req: Request, res: Response, requiredHeaders: string[]): Response | undefined {
    ...
    if (!value || value.trim().length === 0) {
      if (header === "merchant-signature") {
        return sendError(
          ...
          "invalid_request",
          "invalid_signature",
          "Missing Merchant-Signature header."
        );
      }
...
function validateBody(
  validator: ReturnType<typeof createProtocolValidator>,
  req: Request,
  res: Response,
  ...
  if (allowEmptyBody && body && typeof body === "object" && Object.keys(body as object).length === 0) {
    return undefined;
  }
...
function validateResponse(
  ...
  if (!result.valid) {
    ...
    return sendError(
      ...
      "processing_error",
      "invalid_response",
      "Server produced a response that does not match ACP schema"
    );
  }
...
function buildLineItems(
  ...
  sourceItems: unknown,
  currency: string
): { lineItems: Array<Record<string, unknown>>; totals: Array<Record<string, unknown>> } {
...
function buildOrderFromSession(store: InMemoryStore, session: Record<string, unknown>): Record<string, unknown> {
...
function buildCart(store: InMemoryStore, id: string, body: Record<string, unknown>): Record<string, unknown> {
...
function updateCart(store: InMemoryStore, cart: Record<string, unknown>, body: Record<string, unknown>): Record<string, unknown> {
...
function upsertFeedProducts(store: InMemoryStore, feedId: string, products: Array<Record<string, unknown>>): void {
...
export function createAcpApp(): AcpApp {
  const app = express();
  ...
  app.post("/checkout_sessions", (req, res) => {
    const headerError = enforceHeaders(
      req,
      res,
      operationHeaders(headerRequirements, "openapi.agentic_checkout.yaml", "POST", "/checkout_sessions")
    );
    if (headerError) {
      return headerError;
    }
  ...
  app.post("/checkout_sessions/:checkout_session_id/complete", (req, res) => {
    ...
    return validateResponse(
      validator,
      res,
      "checkoutSessionWithOrderResponse",
      completed,
  ...
  app.post("/checkout_sessions/:checkout_session_id/cancel", (req, res) => {
    ...
    if (existing.status === "completed" || existing.status === "canceled") {
      return sendError(
        ...
        "invalid_request",
        "not_cancelable",
        "Checkout session cannot be canceled in its current state"
      );
    }
  ...
  app.post("/carts", (req, res) => {
    const headerError = enforceHeaders(
      req,
      res,
      operationHeaders(headerRequirements, "openapi.cart.yaml", "POST", "/carts")
    );
    if (headerError) {
      return headerError;
    }
  ...
  app.get("/carts/:id", (req, res) => {
    const headerError = enforceHeaders(
      req,
      res,
      operationHeaders(headerRequirements, "openapi.cart.yaml", "GET", "/carts/{id}")
    );
    if (headerError) {
      return headerError;
    }
  ...
  app.put("/carts/:id", (req, res) => {
    const headerError = enforceHeaders(
      req,
      res,
      operationHeaders(headerRequirements, "openapi.cart.yaml", "PUT", "/carts/{id}")
    );
    if (headerError) {
      return headerError;
    }
  ...
  app.post("/carts/:id/cancel", (req, res) => {
    const headerError = enforceHeaders(
      req,
      res,
      operationHeaders(headerRequirements, "openapi.cart.yaml", "POST", "/carts/{id}/cancel")
    );
    if (headerError) {
      return headerError;
    }
  ...
  app.get("/delegate_authentication/:authentication_session_id", (req, res) => {
    ...
    return validateResponse(
      validator,
      res,
      "delegateAuthSessionWithResultResponse",
      payload,
  ...
  app.post("/feeds", (req, res) => {
    const headerError = enforceHeaders(
      req,
      res,
      operationHeaders(headerRequirements, "openapi.feed.yaml", "POST", "/feeds")
    );
    if (headerError) {
      return headerError;
    }
  ...
  app.get("/feeds/:id", (req, res) => {
    const headerError = enforceHeaders(
      req,
      res,
      operationHeaders(headerRequirements, "openapi.feed.yaml", "GET", "/feeds/{id}")
    );
    if (headerError) {
      return headerError;
    }
  ...
  app.get("/feeds/:id/products", (req, res) => {
    const headerError = enforceHeaders(
      req,
      res,
      operationHeaders(headerRequirements, "openapi.feed.yaml", "GET", "/feeds/{id}/products")
    );
    if (headerError) {
      return headerError;
    }
  ...
  app.patch("/feeds/:id/products", (req, res) => {
    const headerError = enforceHeaders(
      req,
      res,
      operationHeaders(headerRequirements, "openapi.feed.yaml", "PATCH", "/feeds/{id}/products")
    );
    if (headerError) {
      return headerError;
    }
    ...
    const body = toObject(req.body);
    const products = (Array.isArray(body.products) ? body.products : []).map((entry) =>
      toObject(entry)
    );
